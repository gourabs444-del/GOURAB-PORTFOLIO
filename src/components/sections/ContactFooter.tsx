...
export function ContactFooter() {
  ...
  return (
    <footer
      id="contact"
      className="relative w-full bg-[#030305] text-white border-t border-white/[0.08] select-none pt-16 sm:pt-24 pb-10 sm:pb-12 px-6 sm:px-12 md:px-16 lg:px-24 overflow-hidden font-sans"
    >
      {/* Top Gradient Laser Accent Line */}
      <div className="pointer-events-none absolute top-0 left-0 right-0 h-[1.5px] bg-gradient-to-r from-transparent via-purple-500/50 via-cyan-500/50 to-transparent shadow-[0_0_16px_rgba(167,139,250,0.5)]" />

      {/* Ambient Radial Glowing Orbs */}
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden select-none">
        <div
          className="absolute -top-32 right-0 w-[600px] h-[500px] rounded-full blur-[140px] opacity-25 bg-[radial-gradient(ellipse_at_center,_rgba(168,85,247,0.3)_0%,_rgba(124,58,237,0.1)_40%,_transparent_70%)]"
          aria-hidden="true"
        />
        <div
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-12 sm:gap-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8 items-start">
          ...
          <div className="md:col-span-5 flex flex-col gap-4">
            <div className="flex items-center gap-3.5">
              {/* Glass Brand Logo Badge */}
              <div className="w-11 h-11 rounded-2xl bg-white/[0.06] backdrop-blur-xl border border-white/10 flex items-center justify-center p-2.5 text-white shadow-[0_4px_16px_rgba(0,0,0,0.4)] hover:border-white/30 hover:bg-white/[0.1] transition-all duration-300">
                <svg
            ...
            {/* Location Indicator */}
            <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-neutral-300 pt-1">
              <div className="flex items-center gap-2 bg-white/[0.04] backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/10 shadow-xs">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
                </span>
                <MapPin className="w-3.5 h-3.5 text-sky-400" />
                <span className="text-neutral-300 font-mono text-[11px] tracking-wider uppercase font-semibold">
          ...
          <div className="md:col-span-3 flex flex-col gap-3.5">
            <h4 className="text-[11px] font-mono font-bold text-neutral-400 uppercase tracking-[0.2em]">
              NAVIGATION
            </h4>
            <ul className="flex flex-col gap-2.5 text-sm font-medium text-neutral-400 font-sans">
              {navLinks.map((item) => (
                <li key={item.id}>
                  <button
                    onClick={() => scrollToSection(item.id)}
                    className="hover:text-white hover:translate-x-1 transition-all duration-200 cursor-pointer text-left font-medium"
                  >
                    {item.label}
                  </button>
                </li>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-1">
              {siteConfig.socials.map((soc) => (
                <a
                  ...
                  aria-label={soc.name}
                  title={soc.name}
                  className="w-11 h-11 rounded-2xl bg-white/[0.04] backdrop-blur-xl border border-white/10 hover:border-white/30 hover:bg-white/[0.1] flex items-center justify-center text-neutral-300 hover:text-white transition-all duration-300 hover:-translate-y-1 hover:scale-105 active:scale-95 cursor-pointer shadow-md"
                >
        ...
        {/* ========================================================= */}
        <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono text-neutral-400 font-sans">
          <div className="flex items-center gap-2.5">
            <div className="w-4 h-4 text-neutral-400 opacity-90 flex items-center justify-center">
              <svg
                viewBox="315 85 394 450"
          ...
          <button
            onClick={scrollToTop}
            className="group inline-flex items-center gap-2 text-neutral-400 hover:text-white transition-colors duration-200 cursor-pointer font-medium"
            aria-label="Back to top"
          >
            <span>Back to top</span>
            <ArrowUp className="w-3.5 h-3.5 text-neutral-400 group-hover:text-white transition-transform duration-200 group-hover:-translate-y-0.5" />
          </button>
        </div>
      </div>
    </footer>
