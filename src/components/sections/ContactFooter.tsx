...
export function ContactFooter() {
  ...
  return (
    <footer
      id="contact"
      className="relative w-full text-slate-900 border-t border-slate-200/60 select-none pt-16 sm:pt-24 pb-10 sm:pb-12 px-6 sm:px-12 md:px-16 lg:px-24 overflow-hidden font-sans"
    >
      {/* Winter Mountain Landscape Artwork Background Canvas */}
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden select-none">
        <div
          className="absolute inset-0 w-full h-full bg-cover bg-bottom bg-no-repeat transition-transform duration-1000 scale-100"
          style={{ backgroundImage: `url('/assets/footer-winter-mountain.png')` }}
        />
        {/* Subtle atmospheric vignette and top light blend */}
        <div className="absolute inset-0 bg-gradient-to-b from-white/75 via-white/40 to-slate-900/20 pointer-events-none" />
      </div>
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-12 sm:gap-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8 items-start">
          ...
          <div className="md:col-span-5 flex flex-col gap-4">
            <div className="flex items-center gap-3.5">
              <div className="w-10 h-10 rounded-xl bg-white/80 backdrop-blur-md border border-slate-300/80 flex items-center justify-center p-2 text-slate-950 shadow-sm hover:border-slate-500 hover:bg-white transition-all duration-300">
                <svg
                  viewBox="315 85 394 450"
                  className="w-full h-full text-slate-950 fill-current stroke-current"
              ...
              <div className="flex flex-col">
                <span className="font-sans font-bold text-lg tracking-tight text-slate-950 leading-none">
                  {siteConfig.name}
                </span>
                <span className="text-xs text-slate-700 font-medium mt-1">
                  Creative Developer &amp; Motion Director
                </span>
              </div>
            </div>

            <p className="font-sans text-sm text-slate-700 font-normal leading-relaxed max-w-sm">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-slate-700 pt-2">
              <div className="flex items-center gap-1.5 bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-slate-300/80 shadow-xs">
                <MapPin className="w-3.5 h-3.5 text-sky-600" />
                <span className="text-slate-800 font-medium">India &bull; Available Worldwide</span>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-3 flex flex-col gap-3.5">
            ...
            <ul className="flex flex-col gap-2.5 text-sm font-medium text-slate-700 font-sans">
              {navLinks.map((item) => (
                <li key={item.id}>
                  <button
                    onClick={() => scrollToSection(item.id)}
                    className="hover:text-sky-600 transition-colors duration-200 cursor-pointer text-left font-medium"
                  >
                    {item.label}
                  </button>
                </li>
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-1">
              {siteConfig.socials.map((soc) => (
                <a
                  ...
                  aria-label={soc.name}
                  title={soc.name}
                  className="w-10 h-10 rounded-xl bg-white/80 backdrop-blur-md border border-slate-300/80 hover:border-sky-500 hover:bg-white flex items-center justify-center text-slate-800 hover:text-sky-600 transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer shadow-sm"
                >
        ...
        {/* ========================================================= */}
        <div className="pt-8 border-t border-slate-300/60 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-medium text-slate-700 font-sans">
          <div className="flex items-center gap-2.5">
            <div className="w-4 h-4 text-slate-800 opacity-90 flex items-center justify-center">
              <svg
                viewBox="315 85 394 450"
          ...
          <button
            onClick={scrollToTop}
            className="group inline-flex items-center gap-2 text-slate-700 hover:text-sky-600 transition-colors duration-200 cursor-pointer font-medium"
            aria-label="Back to top"
          >
            <span>Back to top</span>
            <ArrowUp className="w-3.5 h-3.5 text-slate-700 group-hover:text-sky-600 transition-transform duration-200 group-hover:-translate-y-0.5" />
          </button>
        </div>
      </div>
    </footer>
